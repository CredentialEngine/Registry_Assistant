...
using InputEntity = RA.Models.Input.Job;
using OutputEntity = RA.Models.JsonV2.Job;
...
{
	public class JobServices : ServiceHelperV2
	{
		string className = "JobServices";
		/// <summary>
		/// Publish an Job to the Credential Registry
		/// </summary>
		...
		{
			LoggingHelper.DoTrace( 6, string.Format( "JobServices.Publish Request for: {0} Started.", request.Job.Name ) );
			...
			if ( ToMap( request, output, ref messages ) )
			{
				...
				CER cer = new CER( "Job", output.Type, output.CTID, helper.SerializedInput )
				{
				...
				if ( cer.HasValidPublisherToken() )
				{
					...
					else
					{
						...
						LoggingHelper.DoTrace( 4, string.Format( "Job.Publish. Validate ApiKey failed. Org Ctid: {0}, Document Ctid: {1}, apiKey: {2}", helper.OwnerCtid, output.CTID, cer.PublisherAuthorizationToken ) );
						return; //===================
					}
				}
				else
				 ...
				 */
				if ( !SupportServices.ValidateAgainstPastRequest( "Job", output.CTID, ref cer, ref messages ) )
				{
					isValid = false;
				}
				else
				{
					string identifier = "Job_" + request.Job.CTID;
					...
					if ( cer.Publish( helper, submitter, identifier, ref status, ref crEnvelopeId ) )
					{
						...
						string msg = string.Format( "<p>Published Job: {0}</p><p>Subject webpage: {1}</p><p>CTID: {2}</p> <p>EnvelopeId: {3}</p> ", output.Name, output.SubjectWebpage, output.CTID, crEnvelopeId );
						NotifyOnPublish( "Job", msg );
					}
					else
					{
						messages.Add( status );
		...
		public bool ToMap( EntityRequest request, OutputEntity output, ref List<string> messages )
		{
			...
			try
			{
				CurrentCtid = output.CTID = FormatCtid( input.CTID, "Job Profile", ref messages );
				output.CtdlId = SupportServices.FormatRegistryUrl( ResourceTypeUrl, output.CTID, Community );

				//required
				output.Name = AssignLanguageMap( input.Name, input.Name_Map, "Job.Name", DefaultLanguageForMaps, ref messages, true, 3 );
				...
				output.HasOccupation = AssignRegistryResourceURIsListAsStringList( input.HasOccupation, "HasOccupation", ref messages, false );
				...
				output.Keyword = AssignLanguageMapList( input.Keyword, input.Keyword_Map, "Job Keywords", ref messages );
				//
				...
				output.SameAs = FormatEntityReferencesList( input.SameAs, "Job.Same As", false, ref messages );
				//
		...
		public void HandleCredentialAlignmentFields( InputEntity input, OutputEntity output, ref List<string> messages )
		{
			...
			//output.AlternativeJobType = AssignLanguageMapList( input.AlternativeJobType, input.AlternativeJobType_Map, "Credential AlternativeJobType", ref output.JobType, ref messages );
