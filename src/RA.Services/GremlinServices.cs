...
using System.Web;
using Utilities;
using JM=RA.Models.JsonV2;
using NavyM = RA.Models.Navy.Json;
...
namespace RA.Services
{
	public class GremlinServices
	{
		...
		public List<JM.Agent> IsLearningOppReferencedByCredentials(string ctid, ref List<string> messages)
		{
			messages = new List<string>();
			List<JM.Agent> output = new List<JM.Agent>();

			if( string.IsNullOrWhiteSpace( ctid )  )
			{
				messages.Add( "Error: a valid CTID must be provided." );
			}
				...
				{ "ceterms:requires", new JObject()
					{
						...
						{
							{ "ceterms:ctid","ce-1dbb4ad7-587f-4f7c-95c9-ce885fab17a8" }							
						}
					}
				}
			};
			//should only be one result, but specify higher to catch if extra
			var result = DoQuery( queryData, 0, 2, "", true, null, null );
			var rawResultData = result.Content.ReadAsStringAsync().Result ?? "{}";

			var resultData = JsonConvert.DeserializeObject<CTDLAPIOrganizationResultSet>( rawResultData, new JsonSerializerSettings()
			{
				//Ignore errors
				Error = delegate (object sender, Newtonsoft.Json.Serialization.ErrorEventArgs e) {
					e.ErrorContext.Handled = true;
				}
			} ) ?? new CTDLAPIOrganizationResultSet();
			//need to check result for an org.
		...
		//
		public List<JM.Agent> GetOrgByNameSWP(string name, string swp, ref List<string> messages)
		{
			messages = new List<string>();
			List<JM.Agent> output = new List<JM.Agent>();

			if ( string.IsNullOrWhiteSpace( name ) || string.IsNullOrWhiteSpace(swp) )
			{
				messages.Add( "Error: an organization name and organization webpage must be provided." );
			}
			/*objective
			 * {
				"@type": [
				...
				"search:termGroup": {
					"search:value": [{
						...
						{
							"ceterms:subjectWebpage": "http://www.nocti.org"
						}
					],
			...
			var queryData = new JObject()
			{
				//Get org. expect one or none. TODO add qa
				{ "@type", "ceterms:CredentialOrganization" },
				{ "search:termGroup", new JObject()
					{
						//Where name and swp
						{ "ceterms:name", name },
						{ "ceterms:subjectWebpage", swp },
						{ "search:operator", "search:andTerms" }
					}
				}
			};
		...
		public List<JM.Agent> DoesOrgHaveEntities(string name, string swp, ref List<string> messages)
		{
			messages = new List<string>();
			List<JM.Agent> output = new List<JM.Agent>();
		...
		//
		private SearchResponseMessage DoQuery(JObject query, int skip, int take, string orderBy, bool orderDescending, string referrer = null, string clientIP = null, string descriptionSetType = null)
		{
			...
			var queryJSON = JsonConvert.SerializeObject( requestQuery );

			var queryTargetTypes = GremlinServices.GetQueryMainTargetTypes( query );
			// Prepare the query
			...
			var gremlinQuery = "";
			if ( request.DescriptionSetType == GremlinServices.DescriptionSetType.NoDescriptionSet )
			{
				gremlinQuery = GremlinServices.CTDLQueryToGremlinQuery( query, request.Skip, request.Take, request.OrderBy, request.OrderDescending, context );
			}
			else
			{
				gremlinQuery = GremlinServices.CTDLQueryToDescriptionSetQuery( query, request.Skip, request.Take, request.DescriptionSetType, request.OrderBy, request.OrderDescending, context );
			}

			request.Skip = skip < 0 ? 0 : skip;
			request.Take = take < 0 ? 5 : take > 100 ? 100 : take;
			...
			try
			{
				...
				var totalResults = 0;
				var relatedItems = new List<Dictionary<string, object>>();
				...
				var resultData = GremlinServices.GremlinResponseToDictionarySearchResults( resultContent, ref totalResults, ref relatedItems );

				//Handle the result
				if ( result.IsSuccessStatusCode )
				{
					//Return the results
					return SearchResponse( resultData, true, "okay", new { TotalResults = totalResults, GremlinQuery = consumeRequest.GremlinQuery, RelatedItems = relatedItems } );
				}
				else
				{
					//Forward the error
					return SearchResponse( null, false, "Error: " + result.ReasonPhrase, null );
				}
			...
			{
				return SearchResponse( null, false, ex.Message, null );
			}
		}
		//
		public SearchResponseMessage SearchResponse(object data, bool valid, string status, object extra = null)
		{
			return new SearchResponseMessage( data, valid, status, extra );
		}
		#endregion

		#region Gremlin helpers
		...
		public static List<string> GetCTDLAndCTDLASNContexts()
		{
			...
			if ( contextFiles == null )
			{
				...
				contextFiles.Add( DownloadContextFile( "navy" ) );
		...
		public static string CTDLQueryToGremlinQuery( string jsonText, int skip, int take, string orderBy = "", bool orderDescending = false, GremlinContext context = null )
		{
			...
			var order = SortOrders.DEFAULT;
			try
			{
				order = ( SortOrders ) Enum.Parse( typeof( SortOrders ), orderBy, true );
			}
			catch { }
			return CTDLQueryToGremlinQuery( token, skip, take, order, orderDescending, context );
		}
		//
		...
		public static string CTDLQueryToGremlinQuery( JObject token, int skip, int take, SortOrders orderBy = SortOrders.DEFAULT, bool orderDescending = false, GremlinContext context = null )
		{
			//Initialize context
			...
			var finishPointer = finish;

			switch ( orderBy )
			{
				case SortOrders.NAME:
				finishPointer = finish.SetNext( new GremlinItem( "order().by(coalesce(out('ceterms:name').values(),out('ceasn:name').values(),constant(''))," + (orderDescending ? "decr" : "incr") + ")" ) { IsDirectValue = true, PrintStringValueWithoutQuotes = true, AllowEmptyArguments = true } );
				break;
				case SortOrders.CREATED:
				finishPointer = finish.SetNext( new GremlinItem( "order().by('__created_at'," + (orderDescending ? "decr" : "incr") + ")" ) { IsDirectValue = true, PrintStringValueWithoutQuotes = true, AllowEmptyArguments = true } );
				break;
				case SortOrders.UPDATED:
				finishPointer = finish.SetNext( new GremlinItem( "order().by('__updated_at'," + (orderDescending ? "decr" : "incr") + ")" ) { IsDirectValue = true, PrintStringValueWithoutQuotes = true, AllowEmptyArguments = true } );
				break;
				default: break;
			}
			...
			finishPointer.Next.Next.Next.Arguments.Add( new GremlinItem( "__.values('__payload').fold()" ) { IsDirectValue = true, AllowEmptyArguments = true, PrintStringValueWithoutQuotes = true } ); //Main results
			finishPointer.Next.Next.Next.Arguments.Add( new GremlinItem( "union(__.out().has('@id',test({x,y -> x ==~ y}, /(?i)_:.*/)).values('__payload'),__.out().hasNot('@id').out().has('@id',test({x,y -> x ==~ y}, /(?i)_:.*/)).values('__payload'),__.out().hasNot('@id').out().hasNot('@id').out().has('@id',test({x,y -> x ==~ y}, /(?i)_:.*/)).values('__payload'),__.in('ceasn:isPartOf').values('__payload'),__.in('skos:inScheme').values('__payload')).dedup().fold()" ) { AllowEmptyArguments = true, PrintStringValueWithoutQuotes = true } ); //Try to capture blank nodes
			...
			public override string ToString()
			{
				if ( IsDirectValue )
				{
					...
					if ( DirectValue.Type == JTokenType.String )
					{
						...
						var result = value.Length > 0 ? (PrintStringValueWithoutQuotes ? value : "'" + value + "'") : "";
		...
		public class GremlinContext
		{
			...
			private void ProcessRawContextDocuments( List<string> rawContextDocuments )
			{
				...
				var numberProperties = new List<string>() { "__created_at", "__updated_at" };
		...
		public class DefaultGremlinHandler : IGremlinHandlerSet
		{
			...
			public virtual void ObjectHandler( string property, JObject token, GremlinItem container, GremlinContext context, bool isInArray )
			{
				...
				if ( property == null )
				{
					...
					else if( searchOperator == "search:notTerms" )
					{
						sendToNext = new GremlinItem( "__.not", "" );  //This appears to be the correct syntax, but searches always time out
						container.Arguments.Add( sendToNext );
					}
					//Handling
		...
		public class LanguageMapHandler : DefaultGremlinHandler
		{
			...
			private GremlinItem HasLanguageValues( string propertyName, string textValue, TextMatchType searchMatchType, bool searchCaseSensitive )
			{
				...
				{
					//Filter out encoded payload
					return new GremlinItem( "properties().hasKey(without('__payload')).hasValue", new GremlinItem( WrapTextInRegex( textValue, searchMatchType, searchCaseSensitive ) ) { PrintStringValueWithoutQuotes = true } );
				}
				else
				{
			...
			public override void ValueHandler( string property, JValue token, GremlinItem container, GremlinContext context )
			{
				...
				//Add the properties
				//Filter out encoded payload
				//both('ceterms:name').properties().hasKey(without('__payload')).hasValue('test...')
				wrapper.Next = new GremlinItem( "properties().hasKey(without('__payload')).hasValue", new GremlinItem( wrappedValue ) { PrintStringValueWithoutQuotes = true } );
		...
		public static string CTDLQueryToDescriptionSetQuery( JObject token, int skip, int take, DescriptionSetType dspType, SortOrders orderBy = SortOrders.DEFAULT, bool orderDescending = false, GremlinContext context = null )
		{
			//Initialize context
			context = context ?? new GremlinContext( true );
			...
			switch ( orderBy )
			{
				...
				break;
				case SortOrders.UPDATED:
				finishPointer = finish.SetNext( new GremlinItem( "order().by('__updated_at'," + (orderDescending ? "decr" : "incr") + ").by('__created_at'," + (orderDescending ? "decr" : "incr") + ")" ) { IsDirectValue = true, PrintStringValueWithoutQuotes = true, AllowEmptyArguments = true } );
				break;
				default:
				break;
			}
			...
			var finalUnion = finishPointer.Next.Next.Next;
	...
	public class SearchRequest
	{
		...
		{
			OrderBy = GremlinServices.SortOrders.DEFAULT;
			DescriptionSetType = GremlinServices.DescriptionSetType.NoDescriptionSet;
		}
		public int Skip { get; set; }
		public int Take { get; set; }
		public JToken Query { get; set; }
		public GremlinServices.SortOrders OrderBy { get; set; }
		public bool OrderDescending { get; set; }
		public GremlinServices.DescriptionSetType DescriptionSetType { get; set; } //Optional
	}

	public class SearchResponseMessage : HttpResponseMessage
	{
		public SearchResponseMessage(object data, bool valid, string status, object extra)
		{
	...
	public class CTDLAPIJSONResponse
	{
	...
	public class CTDLAPIJsonResponseExtra
	{
	...
	public class CTDLAPIOrganizationResultSet
	{
		public List<JM.Agent> Results { get; set; }
		public JArray RelatedItems { get; set; }
		public int TotalResults { get; set; }
	}

	public class CTDLAPINavyOrganizationResultSet
	{
		public List<NavyM.Organization> Results { get; set; }
		public JArray RelatedItems { get; set; }
		public int TotalResults { get; set; }
	}
}
