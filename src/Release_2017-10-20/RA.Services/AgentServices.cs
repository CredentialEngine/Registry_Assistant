...
namespace RA.Services
{
	public class AgentServices : ServiceHelper
	{
		...
		public static bool HandleRequiredFields( InputEntity input, OutputEntity output, ref List<string> messages )
		{
			...
			{
				messages.Add( "Error - An @Id must be provided/generated from CTID." );
			}
			//required
		...
		public static void HandleLocations( InputEntity input, OutputEntity output, ref List<string> messages )
		{
			...
			//17-06-14 MP - decided output not make email required
			...
			if ( input.Address != null || input.Address.Any() )
			{
				foreach ( var address in input.Address )
				{
					output.Address.Add( FormatAddress( address, ref messages ) );
				}
			}
			else
				output.Address = null;

			if ( input.ContactPoint != null || input.ContactPoint.Any() )
			{
		...
		{
			output.Image = AssignValidUrlAsStringList( input.Image, "Image", ref messages );
		...
		public static void HandleCredentialAlignmentFields( InputEntity from, OutputEntity to, ref List<string> messages )
		{
			...
			if ( from.AgentType != null && from.AgentType.Count > 0 )
			{
				foreach ( var item in from.AgentType )
				{
					//to.AgentType.Add( FormatCredentialAlignment( item ) );
					to.AgentType.Add( FormatCredentialAlignment( "agentType", item, ref messages ) );
				}
			}
			else
			{
			...
			if ( !string.IsNullOrWhiteSpace(from.AgentSectorType))
			{
				//foreach ( string item in from.AgentSectorType )
				//{
					to.AgentSectorType.Add( FormatCredentialAlignment( "agentSectorType", from.AgentSectorType, ref messages ) );
				//}
			}
			else
			{
				messages.Add( "Error = Agent Sector Type is a required property. " );
				to.AgentSectorType = null;
			}
